...
MovesListNode *__generate_pawn_moves(Position *position, uint8_t src,
  ...
  for (uint8_t i = 0; i < 2; i++) {
    ...
    if (POS_IS_SQ_ON_BOARD(dest) &&
        ...
        dest == position->enpassant_target && POS_IS_SQ_EMPTY(position, dest)) {
...
MovesListNode *__generate_knight_moves(Position *position, uint8_t src,
                                       MovesListNode *moves_list_tail) {
...
MovesListNode *__generate_bishop_moves(Position *position, uint8_t src,
                                       MovesListNode *moves_list_tail) {
...
MovesListNode *__generate_rook_moves(Position *position, uint8_t src,
                                     MovesListNode *moves_list_tail) {
...
MovesListNode *__generate_queen_moves(Position *position, uint8_t src,
                                      MovesListNode *moves_list_tail) {
...
MovesListNode *__generate_king_moves(Position *position, uint8_t src,
                                     MovesListNode *moves_list_tail) {
  ...
  if (POS_ACTIVE_COLOR(position) == WHITE) {
    if (position->casteling_rights & WKCA) {
      if (POS_IS_SQ_EMPTY(position, POS_RF_TO_SQ(RANK_1, FILE_F)) &&
        ...
        move = encode_king_castle_move(king, POS_RF_TO_SQ(RANK_1, FILE_E),
                                       POS_RF_TO_SQ(RANK_1, FILE_G),
                                       MOVE_FLAG_WKCA);
        new_moves_list_tail = __insert_move_to_list(new_moves_list_tail, move);
      }
    }
    ...
    if (position->casteling_rights & BKCA) {
      if (POS_IS_SQ_EMPTY(position, POS_RF_TO_SQ(RANK_8, FILE_F)) &&
        ...
        move = encode_king_castle_move(king, POS_RF_TO_SQ(RANK_8, FILE_E),
                                       POS_RF_TO_SQ(RANK_8, FILE_G),
                                       MOVE_FLAG_BKCA);
        new_moves_list_tail = __insert_move_to_list(new_moves_list_tail, move);
      }
    }
...
MovesListNode *generate_moves_list(Position *position) {
  ...
  for (uint8_t i = 21; i < 99; i++) {
    if (PIECE_GET_COLOR(POS_PIECE(position, i)) != POS_ACTIVE_COLOR(position)) {
      continue;
    }

    if (PIECE_GET_TYPE(POS_PIECE(position, i)) == PAWN) {
      moves_list_tail = __generate_pawn_moves(position, i, moves_list_tail);
    } else if (PIECE_GET_TYPE(POS_PIECE(position, i)) == KNIGHT) {
      moves_list_tail = __generate_knight_moves(position, i, moves_list_tail);
    } else if (PIECE_GET_TYPE(POS_PIECE(position, i)) == BISHOP) {
      moves_list_tail = __generate_bishop_moves(position, i, moves_list_tail);
    } else if (PIECE_GET_TYPE(POS_PIECE(position, i)) == ROOK) {
      moves_list_tail = __generate_rook_moves(position, i, moves_list_tail);
    } else if (PIECE_GET_TYPE(POS_PIECE(position, i)) == QUEEN) {
      moves_list_tail = __generate_queen_moves(position, i, moves_list_tail);
    } else if (PIECE_GET_TYPE(POS_PIECE(position, i)) == KING) {
      moves_list_tail = __generate_king_moves(position, i, moves_list_tail);
    }
  }
